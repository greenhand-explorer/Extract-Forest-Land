...
def HowManyForest(input_file, forest_code, nodata_code=65535):
    """统计林地占比
    ...
    dpix=dataset.GetGeoTransform()[1]
    dpiy=abs(dataset.GetGeoTransform()[5])
...
for area in range(1,4):
    ...
    for year in range(2021,2024):
        File='zone/Classified/00%d%d0_classified.tif'%(area,year)
        dataset=gdal.Open(File,gdal.GA_ReadOnly)
        XSize=dataset.RasterXSize
        YSize=dataset.RasterYSize
        array=dataset.ReadAsArray(0,0,XSize,YSize)
        array[array==65535]=10
        array[array==0]==10
